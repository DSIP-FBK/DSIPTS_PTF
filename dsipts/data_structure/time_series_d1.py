    ...
    if group_cols:
        ...
        for group_name, group_data in df.groupby(group_cols_list):
            ...
            if isinstance(group_name, tuple):
                ...
                # Single group column
                group_df = pd.DataFrame({time_col: time_range, group_cols_list[0]: group_name})
...
class MultiSourceTSDataSet(Dataset):
    ...
    def _preload_data(self):
            ...
            for col in self.cat_cols:
                if col in file_data.columns:
                    file_data[col] = (
                        ...
                        .transform(file_data[col].values.reshape(-1, 1))
                        .flatten()
                    )
        ...
        if self.data_cache:
            total_mb = sum(df.memory_usage(deep=True).sum() for df in self.data_cache.values()) / (
                1024 * 1024
            )
            ...
            if col in data.columns:
                ...
                if len(unique_values) > 0:
                    if col not in self.label_encoders:
                        ...
                        new_cats = np.unique(
                            np.concatenate([current_cats, unique_values.flatten()])
                        )
    ...
    def _prepare_metadata(self):
        ...
        self.metadata = {
            "cols": {"y": self.target_cols, "x": self.feature_cols, "st": self.static_cols},
            "col_type": {},
            "col_known": {},
            "weight": self.weights,
            "cat_index": [],
        ...
        self.metadata["known_num_cols"] = [
            col for col in self.known_cols if col not in self.cat_cols
        ]
        self.metadata["unknown_cat_cols"] = [
            col for col in self.unknown_cols if col in self.cat_cols
        ]
        self.metadata["unknown_num_cols"] = [
            col for col in self.unknown_cols if col not in self.cat_cols
        ]
                    ...
                    if mask.any():
                        ...
                        for col in self.cat_cols:
                            if col in group_chunk.columns:
                                group_chunk[col] = (
                                    self.label_encoders[col]
                                    .transform(group_chunk[col].values.reshape(-1, 1))
                                    .flatten()
                                )
