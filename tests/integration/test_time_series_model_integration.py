...
def convert_batch_for_model(
    batch, known_cat_cols=None, known_num_cols=None, unknown_cat_cols=None, unknown_num_cols=None
):
    """
...
def main():
    try:
        ...
        logger.info(
            f"Creating D1 dataset with: time_col={time_col}, target_col={target_col}, "
        ...
        d1 = MultiSourceTSDataSet(
            ...
            + ["hour", "day_of_week", "month"],  # Include categorical features
        ...
        logger.info("Created SimpleTimeSeriesModel")
        ...
        assert (
            model_batch["x_num_past"].shape[0] == model_batch["x_cat_past"].shape[0]
        ), "Batch size mismatch between numerical and categorical features"
        assert (
            model_batch["x_num_future"].shape[1] == model_batch["y"].shape[1]
        ), "Future sequence length mismatch"
